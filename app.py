...
st.title("Market Basket Analysis dan Association Rules")
...
if uploaded_file is not None:
    ...
    txs = database.groupby(['id'])['produk'].apply(lambda x: list(np.unique(x)))
    ...
    product_max_Top10 = product_max.sort_values("numVeces", ascending=False).head(5)
    ...
    b.set_title("Top 5 Produk Terlaris", fontsize=30)
    ...
    fpgrowth_df = fpgrowth(df, min_support=min_support, use_colnames=True)
    sorted_fpgrowth_df = fpgrowth_df.sort_values(by='support', ascending=False)
    # Convert frozenset to string for display
    sorted_fpgrowth_df['itemsets'] = sorted_fpgrowth_df['itemsets'].apply(lambda x: ', '.join(list(x)))

    # Display sorted_apriori_df
    st.subheader("Products (Sorted by Support)")
    st.text("Support adalah nilai suatu produk yang sering muncul")
    st.write(sorted_fpgrowth_df)

    rules_df = association_rules(fpgrowth_df, metric="confidence", min_threshold = min_threshold)
    # Convert frozenset to string for display
    ...
    st.text("Confidence adalah nilai keterkaitan antara satu produk dengan produk yang lain")
    st.write(rules_df)
else:
